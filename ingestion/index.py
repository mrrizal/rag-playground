...
class BGEEmbeddingFunction(EmbeddingFunction):
    ...
    def __call__(self, texts: List[str]) -> List[List[float]]:
        # Optionally, prepend "passage: " as recommended by the model
        return self.model.encode(
            [f"passage: {text}" for text in texts],
            show_progress_bar=False,
            normalize_embeddings=True,  # important for ChromaDB!
        )
...
class ChromaDBIndexingService(IndexingService):
    ...
    def __init__(self, collection_name: str = "code_repository", persist_directory: str = "./chroma_db"):
        ...
        if not ChromaDBIndexingService._client:
            ...
            ChromaDBIndexingService._client = chromadb.PersistentClient(
    ...
    def prepare_documents_for_indexing(self, chunks: List[Dict[str, Any]]) -> tuple:
        ...
        for chunk in chunks:
            # Skip empty chunks
            raw_code = chunk.get("code", "").strip()
            if not raw_code:
                continue

            normalized_code = chunk.get("normalized_code", "").strip()

            context_parts = [normalized_code]
